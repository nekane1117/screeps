...
// constants.ts
var TERMINAL_LIMIT, TERMINAL_THRESHOLD, LAB_STRATEGY, REVERSE_REACTIONS, ALL_REACTIONS, ROAD_DECAY_AMOUNT_SWAMP, ROAD_DECAY_AMOUNT_WALL, DECOMPRESSING_COMMODITIES, COMPRESSING_INGREDIENT;
var init_constants = __esm({
  "constants.ts"() {
    "use strict";
    init_utils_common();
    TERMINAL_LIMIT = 1e4;
    TERMINAL_THRESHOLD = 1e3;
    LAB_STRATEGY = {
      [RESOURCE_LEMERGIUM]: [
        RESOURCE_LEMERGIUM,
        RESOURCE_HYDROGEN,
    ...
    ALL_REACTIONS = _(ObjectKeys(REVERSE_REACTIONS)).sortBy((r) => r === "G" ? 0 : r.length);
    ROAD_DECAY_AMOUNT_SWAMP = 500;
    ROAD_DECAY_AMOUNT_WALL = 15e3;
    DECOMPRESSING_COMMODITIES = [
      RESOURCE_UTRIUM,
      RESOURCE_LEMERGIUM,
      RESOURCE_ZYNTHIUM,
      RESOURCE_KEANIUM,
...
var squareDiff, DIRECTIONS, IDEAL_BODY, RETURN_CODE_DECODER, customMove, DEFAULT_CREEP_RANGE, REVERSE_BOOSTS;
var init_util_creep = __esm({
  "util.creep.ts"() {
    "use strict";
    init_util_array();
    init_utils();
    squareDiff = Object.freeze([
      [-1, -1],
      [0, -1],
      [1, -1],
      [-1, 0],
    ...
    RETURN_CODE_DECODER = Object.freeze({
      [OK.toString()]: "OK",
...
var init_flags = __esm({
  "flags.ts"() {
    ...
    flags_default = {
      [COLOR_RED]: behavior6,
      [COLOR_WHITE]: behavior7,
      [COLOR_PURPLE]: behavior8
    };
  }
});
...
var behavior9, role_carrier_default;
var init_role_carrier = __esm({
  "role.carrier.ts"() {
    "use strict";
    init_constants();
    init_util_creep();
    ...
    behavior9 = (creep) => {
      ...
      function checkMode() {
        ...
        const newMode = ((c) => {
          ...
          if (c.memory.mode === "\u{1F6D2}" && creep.store.energy >= Math.min(creep.store.getCapacity(RESOURCE_ENERGY), creep.room.controller ? EXTENSION_ENERGY_CAPACITY[creep.room.controller.level] : CARRY_CAPACITY)) {
            return "\u{1F69B}";
          }
      ...
      if (!creep.memory.storeId) {
        creep.memory.storeId = (_a = (() => {
          ...
          return extructor && extructor.store.energy >= CARRY_CAPACITY ? extructor : void 0;
        })()) == null ? void 0 : _a.id;
      }
      if (!creep.memory.storeId && room.energyAvailable < room.energyCapacityAvailable) {
        creep.memory.storeId = (_b = creep.pos.findClosestByRange(_.compact([room.storage, ...containers]), {
          filter: (s) => {
            return (containers.length < 2 || (controllerContaeiner == null ? void 0 : controllerContaeiner.id) !== s.id) && s.store.energy > 0;
          }
        })) == null ? void 0 : _b.id;
      }
      if (!creep.memory.storeId && room.energyAvailable < room.energyCapacityAvailable && (((_c = room.terminal) == null ? void 0 : _c.store.energy) || 0) >= CARRY_CAPACITY) {
        creep.memory.storeId = (_d = room.terminal) == null ? void 0 : _d.id;
      }
      if (!creep.memory.storeId) {
        creep.memory.storeId = (_e = creep.pos.findClosestByRange(containers, {
          filter: (s) => {
            return (containers.length < 2 || (controllerContaeiner == null ? void 0 : controllerContaeiner.id) !== s.id) && s.store.energy >= CARRY_CAPACITY;
          }
        })) == null ? void 0 : _e.id;
      }
      if (!creep.memory.storeId) {
      ...
      if (!creep.memory.storeId) {
        const storageOrHarvester = creep.room.storage || creep.room.terminal || creep.pos.findClosestByRange(getCreepsInRoom(creep.room).harvester || [], {
          filter: (c) => {
            return c.store.energy > ((c == null ? void 0 : c.getActiveBodyparts(WORK)) || 0) * BUILD_POWER;
          }
        });
      ...
      if (!creep.memory.transferId) {
        creep.memory.transferId = (_h = _(getCreepsInRoom(creep.room).builder || []).compact().sortBy((s) => s.store.energy).first()) == null ? void 0 : _h.id;
      }
      if (!creep.memory.transferId) {
      ...
      if (creep.memory.transferId && creep.memory.mode === "\u{1F69B}") {
        ...
        if (transferTarget) {
          ...
          if (creep.pos.isNearTo(transferTarget)) {
            ...
            _([...extension, ...spawns].filter((e) => creep.pos.isNearTo(e) && e.store.getFreeCapacity(RESOURCE_ENERGY))).tap(([head]) => {
              if (head) {
    ...
    role_carrier_default = behavior9;
  }
});
...
var behavior10, role_builder_default, boosts;
var init_role_builder = __esm({
  "role.builder.ts"() {
    "use strict";
    init_role_carrier();
    init_util_creep();
    ...
    role_builder_default = behavior10;
    boosts = [RESOURCE_CATALYZED_LEMERGIUM_ACID, RESOURCE_LEMERGIUM_ACID, RESOURCE_LEMERGIUM_HYDRIDE];
  }
});
...
var behavior11, role_claimer_default;
var init_role_claimer = __esm({
  "role.claimer.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    ...
    role_claimer_default = behavior11;
  }
});
...
var behavior12, role_defender_default;
var init_role_defender = __esm({
  "role.defender.ts"() {
    "use strict";
    init_util_creep();
    behavior12 = (creep) => {
    ...
    role_defender_default = behavior12;
  }
});
...
var behavior13, role_harvester_default;
var init_role_harvester = __esm({
  "role.harvester.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    ...
    role_harvester_default = behavior13;
  }
});
...
var TRANSFER_THRESHOLD2, behavior14, role_labManager_default;
var init_role_labManager = __esm({
  "role.labManager.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    TRANSFER_THRESHOLD2 = 1e3;
    behavior14 = (creep) => {
      var _a, _b, _c, _d, _e;
      const { room } = creep;
          ...
          if (structure.structureType === STRUCTURE_FACTORY) {
            ...
            if (structure.mineralType) {
              if (structure.mineralType !== structure.memory.expectedType) {
                ...
                if (structure.store[structure.mineralType] > TRANSFER_THRESHOLD2 * 2) {
                  mapping.completed.push(structure);
                } else if (structure.store[structure.mineralType] <= TRANSFER_THRESHOLD2) {
                  mapping.requesting.push(structure);
                } else {
              ...
              if (store.structureType === STRUCTURE_TERMINAL) {
                ...
                if (memory.outputType) {
                  return creep2.withdraw(
                    store,
                    memory.outputType,
                    Math.min(creep2.store.getCapacity(memory.outputType), Math.max(0, store.store[memory.outputType] - TRANSFER_THRESHOLD2 * 2))
                  );
                } else {
                  creep2.memory.storeId = void 0;
                  creep2.memory.mineralType = void 0;
    ...
    role_labManager_default = behavior14;
  }
});
...
var behavior15, role_mineralCarrier_default;
var init_role_mineralCarrier = __esm({
  "role.mineralCarrier.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    ...
    role_mineralCarrier_default = behavior15;
  }
});
...
var behavior16, role_mineralHarvester_default;
var init_role_mineralHarvester = __esm({
  "role.mineralHarvester.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
        ...
        mineral.pos.findInRange(
          [
            ...
            ...Object.values(Game.constructionSites).filter(
              (s) => s.pos.roomName === creep.pos.roomName && s.structureType === STRUCTURE_CONTAINER
            )
          ],
          2
        )
    ...
    role_mineralHarvester_default = behavior16;
  }
});
...
var behavior17, role_remoteCarrier_default;
var init_role_remoteCarrier = __esm({
  "role.remoteCarrier.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    behavior17 = (creep) => {
      ...
      if (memory.mode === "\u{1F69B}") {
        ...
        if (creep.getActiveBodyparts(WORK) === 0) {
          return creep.memory.mode = "\u{1F69B}";
        }
      ...
      if (creep.memory.mode === "\u{1F69B}" && creep.pos.roomName !== creep.memory.baseRoom && getSitesInRoom(creep.room).length === 0 && !isHighway(creep.room) && !creep.pos.lookFor(LOOK_STRUCTURES).find((s) => s.structureType === STRUCTURE_ROAD)) {
        creep.pos.createConstructionSite(STRUCTURE_ROAD);
      }
      pickUpAll(creep);
    };
    role_remoteCarrier_default = behavior17;
  }
});
...
var behavior18, role_remoteHarvester_default;
var init_role_remoteHarvester = __esm({
  "role.remoteHarvester.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    behavior18 = (creep) => {
      ...
      } else if (creep.memory.mode === "\u{1F33E}" && creep.store.getFreeCapacity(RESOURCE_ENERGY) === 0 && getSitesInRoom(creep.room).length) {
        creep.memory.mode = "\u{1F477}";
    ...
    role_remoteHarvester_default = behavior18;
  }
});
...
var behavior19, role_reserver_default;
var init_role_reserver = __esm({
  "role.reserver.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    ...
    role_reserver_default = behavior19;
  }
});
...
var behavior20, role_upgrader_default, changeMode;
var init_role_upgrader = __esm({
  "role.upgrader.ts"() {
    "use strict";
    init_util_creep();
    init_utils();
    ...
    role_upgrader_default = behavior20;
    changeMode = (creep, mode) => {
...
var init_roles = __esm({
  "roles.ts"() {
    ...
    init_role_defender();
    init_role_harvester();
    init_role_labManager();
    init_role_mineralCarrier();
    init_role_mineralHarvester();
    init_role_remoteCarrier();
    init_role_remoteHarvester();
    init_role_reserver();
    init_role_upgrader();
    behaviors4 = {
  ...
  if (checkSpawnBuilder(room)) {
    const spawn = (() => {
      ...
      if (spawns.length > 0) {
        ...
        return (_a2 = room.controller) == null ? void 0 : _a2.pos.findClosestByPath(Object.values(Game.spawns));
      }
    })();
...
var structure_controller_default = behavior4;
function isC(s) {
...
function behavior5(extractor) {
  ...
  if (container.find((s) => s.store[mineral.mineralType] > 0) && mineralCarrier.length < 1) {
    const spawn = _(getSpawnsInRoom(extractor.room)).filter((s) => !s.spawning).first();
    if (!spawn) {
      console.log(`source ${extractor.id} can't find spawn`);
      return ERR_NOT_FOUND;
    }
  ...
  logUsage("all", () => {
    ...
    logUsage("flags", () => {
      Object.values(Game.flags).forEach((flag) => {
        var _a, _b;
        return (_b = (_a = (init_flags(), __toCommonJS(flags_exports)))[flag.color]) == null ? void 0 : _b.call(_a, flag);
      });
    });
    logUsage("rooms", () => {
    ...
    logUsage("creep", () => {
      Object.values(Game.creeps).forEach((c) => {
        ...
        (_b = (_a = (init_roles(), __toCommonJS(roles_exports)).behaviors)[c.memory.role]) == null ? void 0 : _b.call(_a, c);
