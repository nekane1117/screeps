/* global FIND_STRUCTURES, STRUCTURE_SPAWN, STRUCTURE_EXTENSION, STRUCTURE_TOWER, STRUCTURE_CONTAINER, STRUCTURE_STORAGE, Room, Game, ERR_NOT_FOUND, FIND_MY_STRUCTURES, RESOURCE_ENERGY, FIND_MY_SPAWNS, ERR_FULL, _, FIND_MY_CONSTRUCTION_SITES, OK, STRUCTURE_ROAD, STRUCTURE_WALL */
/*
		...
		if (target === null) {
			...
			if (target.structureType === STRUCTURE_TOWER && creep.pos.inRangeTo(target, myConst.TRANSFER_RANGE)) {
				...
				if (OK !== creep.transfer(target, RESOURCE_ENERGY)) {
					//失敗したらなんかアラートを出す
					console.log(creep.name + ' TRANSFER FAILD!' + target.pos);
				}
				//初期化(毎回)
				creep.memory.target = null;
				...
				if (OK !== creep.repair(target)) {
					//失敗したらなんかアラートを出す
					console.log(creep.name + 'REPAIRING FAILD!' + target.pos);
				}
				//初期化(毎回)
				creep.memory.target = null;
			} else {
